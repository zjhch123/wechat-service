...
describe('wx-code-auth.js', () => {
  it('should return successful response', async () => {
    const response = await request.get(`/wxCodeAuth?code=${mockSuccessCode}&redirect_uri=http%3A%2F%2F127.0.0.1:7610&service_id=dev`);
  ...
  it('should return error if missing params', async () => {
    ...
    expect(response.text).to.equal('Missing params: code, redirect_uri, service_id');
  });
});
